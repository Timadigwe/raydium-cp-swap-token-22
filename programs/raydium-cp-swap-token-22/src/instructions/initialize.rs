...
pub fn initialize(
    ...
    msg!("Starting initialize function");
    msg!("init_amount_0: {}, init_amount_1: {}", init_amount_0, init_amount_1);
    ...
    msg!("Token_0_vault account created successfully");

    msg!("Creating token_1_vault account");
    create_token_account(
        &ctx.accounts.authority.to_account_info(),
        &ctx.accounts.creator.to_account_info(),
    ...
    msg!("Token_1_vault account created successfully");

    msg!("Creating pool state");
    ...
    msg!("Token_0 transfer completed");

    msg!("Transferring token_1 from user to vault");
    transfer_from_user_to_pool_vault(
        ctx.accounts.creator.to_account_info(),
    ...
    msg!("Token_1 transfer completed");

    msg!("Reading vault amounts");
    let token_0_vault =
    ...
    msg!("Vault amounts - token_0_vault.amount: {}, token_1_vault.amount: {}", 
         token_0_vault.amount, token_1_vault.amount);

    CurveCalculator::validate_supply(token_0_vault.amount, token_1_vault.amount)?;
    msg!("Supply validation passed");
    ...
    let lp_amount_to_mint = liquidity.checked_sub(lock_lp_amount);
    msg!("lp_amount_to_mint: {:?}", lp_amount_to_mint);
    
    if lp_amount_to_mint.is_none() {
        msg!("ERROR: liquidity ({}) is less than lock_lp_amount ({})", liquidity, lock_lp_amount);
    ...
    token::token_mint_to(
        ...
        lp_amount_to_mint.unwrap(),
        &[&[crate::AUTH_SEED.as_bytes(), &[ctx.bumps.authority]]],
    )?;
    msg!("LP tokens minted successfully");

    // Charge the fee to create a pool
    if ctx.accounts.amm_config.create_pool_fee != 0 {
        msg!("Charging create pool fee: {}", ctx.accounts.amm_config.create_pool_fee);
        invoke(
        ...
        msg!("Create pool fee charged successfully");
    } else {
        msg!("No create pool fee to charge");
    }
    ...
    msg!("Pool state initialized successfully");

    msg!("Initialize function completed successfully");
    Ok(())
}
